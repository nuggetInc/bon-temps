            ...
            <table class="table table-striped table-hover shadow-sm w-auto mb-5">
                <thead>
                    <tr>
                        ...
                        <th class="text-end"><button type="submit" name="add" title="Nieuw gerecht toevoegen" class="fa-solid fa-square-plus display-1 btn btn-lg p-1"></button></th>
                    </tr>
                </thead>
                <tbody>
                    <?php for ($index = 0; $index < $_SESSION["amount"]; $index++) : ?>
                        <tr>
                            ...
                            <td class="px-3">
                                <select name="dishID<?= $index ?>" class="form-select">
                                    <?php

                                    $dishID = $_SESSION["dishID$index"];

                                    if ($dishID === 0) {
                                        $dishName = "Kies een gerecht";
                                    } else {
                                        $dishName = $dishes[$_SESSION["dishID$index"]]->getName();
                                    }
                            ...
                            <td class="px-3" colspan="2"><input type="number" name="amount<?= $index ?>" class="form-control" value="<?= $_SESSION["amount$index"]  ?>" min="0" /></td>
                        </tr>
                    <?php endfor ?>
                </tbody>
            </table>
